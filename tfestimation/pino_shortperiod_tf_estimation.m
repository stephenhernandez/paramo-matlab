% pino transfer function estimation
clear; close all;
%the gain on the PINO BB station changed on 20 Aug 2024, so only use after 21 Aug...
dayStart = datetime(2024,08,16);
dayEnd = datetime(2026,01,10);
dayInc = 1;
...
for i = 1:lDays
    day_ = dayVec(i);
    % PINO_SH = loadWaveforms(day_,dayInc,"PINO",["SHZ";"SHN";"SHE"],"EC",""); %,0,0,"~/masa/backups");
    ...
    for j = 2
        ...
        % Sf(1) = transferWaveforms(Sf(1),newFs/nfft,-inf,npoles,newFs,"vel",true,false,...
...
fRef = 2;
fRef_index = find(fVec >= fRef,1);
fRef = fVec(fRef_index);
axL = linkedPlot(tFilt,[med_abs_decon phase(fRef_index,:)'],"o","compact"); 
hold(axL(1),"on");
plot(axL(1),tFilt(mI),med_abs_decon(mI),'p'); zoom on; grid on; hold on;
hold(axL(2),"on");
plot(axL(2),tFilt,medfiltSH(phase(fRef_index,:)',24,true),'p'); zoom on; grid on; hold on;
axL(1).YScale = "log";
