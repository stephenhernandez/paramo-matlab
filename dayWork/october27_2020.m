...
%%
tStart = datetime(2020,10,27); %datetime(2012,11,08);
...
for i = 1:length(days)
    ...
    %%
    S = loadWaveforms(day_,1,"TOMA","BHZ");
    S(2,1) = loadWaveforms(day_,1,"SUCR","BHZ");
...
S = loadWaveforms(datetime(2020,01,14),01,"TOMA","BHZ");
S = resampleWaveforms(S,newFs);
S = filterWaveforms(S,lfc,hfc,npoles);
tt = getTimeVec(S);
dd = S.d;
...
S = loadWaveforms(datetime(2020,01,14),01,"SUCR","BHZ");
S = resampleWaveforms(S,newFs);
S = filterWaveforms(S,lfc,hfc,npoles);
tt = getTimeVec(S);
dd = S.d;
