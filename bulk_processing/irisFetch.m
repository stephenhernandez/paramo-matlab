   ...
   methods(Static, Access=protected)
      ...
      function [ tr_out ] = read_sac_in(fileDirectory)
         ...
         elseif exist(fileDirectory,'dir')==7  % ..for directories..
             ...
             cd(fileDirectory)
             fileNames = dir('*.SAC');
         elseif  logical(strfind(fileDirectory,'*')) || contains(fileDirectory,'?') % ..for wildcards..
          ...
          function trace_out = format_blank_fields(trace_in)
             ...
             for ii = 1:numel(names)
                 if isnumeric(trace_in.(names{ii})) && trace_in.(names{ii})==-12345
                     trace_in.(names{ii}) = [];
                 elseif ischar(trace_in.(names{ii})) && contains(trace_in.(names{ii}),'-12345')
                     trace_in.(names{ii}) = [];
                 end
             end
             trace_out = trace_in;
