function [multipletI,lPerMultiplet,multipletNumber,tree] = returnClusts(D,thresh,method)
% D = distance vector
% thresh = threshold
...
multipletNumber = cluster(tree,"cutoff",cutoff,...
    "criterion","distance");

uniqueT = unique(multipletNumber);
lPerMultiplet = groupcounts(multipletNumber);
nMultiplets = length(uniqueT);

multipletI = cell(nMultiplets,1);
for i = 1:nMultiplets
    thisMultiplet = uniqueT(i);
    uniq_tmps = find(multipletNumber == thisMultiplet);
