...
normWeights = 1./sum(weights,3);
weights = weights.*normWeights;
Syy = sum(weights.*Syy,3);

ampsxx = real(median(sqrt(Sxx),1,"omitnan"))';
ampsyy = real(median(sqrt(Syy),1,"omitnan"))';
badI = ~isfinite(ampsxx) | ampsxx == 0 | ampsxx >= median(ampsxx) | ampsyy >= median(ampsyy); %~isfinite(ampsyy) | ampsyy == 0;
...
%%
[Syy,sI] = sort(Syy,2);
%[mscohe,sI] = sort(mscohe,2,"descend");
