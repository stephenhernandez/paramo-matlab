function [ll,tabs,energyRatio,Neff,z2p,p2rms,kurt,skew,ReconstructCoefficients,...
    ...
    multiplexedSubspaceDetector(S,varargin)
...
S = syncWaveforms(detrendWaveforms(S),false,true,true);
...
for i = 1:lB
    ...
    theseU = pull(B(:,i)); %<-- pull all basis functions for station i (they're pre-multiplexed)
    theseU = flipud(theseU);
    winlen = size(theseU,1);
    box = ones(winlen,1);

    dLong_ = dLong_';
    dLong_ = dLong_(:); %<-- multiplex 3 components of station i
    ...
    snorms = norms > 1;
    cc_ = fftfilt(theseU,dLong_);
    cc_ = sum(cc_.^2,2); %sum energy contributions across all nBF basis functions (this station only)
    eratio_ = snorms.*cc_./norms;
...
%% compress ccnorm
goodI = isfinite(eratio);
nEffective = sum(goodI,2);
...
%%
badI = ~isfinite(eratio);
eratio(badI) = 0; % fix NaNs
