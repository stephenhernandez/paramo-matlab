function [t,eqlat,eqlon,eqdepth,eqmag,id,D,ax] = genFrames(varargin)
% Example: [t,eqlat,eqlon,eqdepth,eqmag,id,D] =
% generateSeismicityAnimationFrames(tStart,tEnd,regionName,boundaryBox,
% SC5Flag,minMag, depthPanel,cumPanel,minDepth,maxDepth,addSimbologia,
% allFrames,hillShadeFlag,magFact,fontSize,fstring,minLevInc,diasFlag,depthRange,refVMdepth);
...
if SC5Flag
    ...
    if isstring(tStart)
        fullfilename = tStart;
        [t,eqlat,eqlon,eqdepth,eqmag,id] = readHypoDD(fullfilename,true);
        tStart = dateshift(min(t),"start","da");
    else
...
if strcmp(regionName,'fernandina') || strcmp(regionName,'galapagos')
    ...
    S = shaperead('~/igdata/Fernandina/Fernandina2018.shp','UseGeoCoords',true);
    ...
    S = shaperead('~/igdata/Fernandina/Fernandina2017.shp','UseGeoCoords',true);
    [slat,slon] = utm2ll(S.Lon,S.Lat,15);
    %figure(1); hold on;
    plot(ax(laxes),slon,slat,'r','linewidth',2); zoom on; grid on;
    axis equal;
    S = shaperead('~/igdata/Fernandina/Fernandina2020_3mar','UseGeoCoords',true);
...
for i = lDI
    ...
    ax(laxes+1) = axes(TL); %nexttile(1,[2,2]);
    ax(laxes+1).Layout.Tile = 1;
    ax(laxes+1).Layout.TileSpan = [2 2];
    hold(ax(laxes+1),'on');
    if timeColorCode
    ...
    if LT
        ...
        titleStr = [datestr(t(dI(i))),' (UTC), N=',num2str(i)];
    end
    ...
    fname = sprintf('~/animations/frame_%04d',i);

    % upper right panel
    if depthPanel
        ax(laxes+2) = nexttile(5);
        if timeColorCode
