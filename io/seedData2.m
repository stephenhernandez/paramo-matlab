function S = seedData2(dayToLoad,tStart,tEnd,HORFLAG,verboseFlag,chans,nets,kholes,rawDataDir)
if nargin < 2
    tStart = NaT(1);
end
...
for i = 1:lNets
    ...
    for j = 1:lkstnmNames
        ...
        for k = 1:lChanDirNames
            ...
            for kk = 1:lkholes
                khole_ = kholes(kk);
                fileStr = sprintf("%s.%s.%s.%s.%03d",net_,thisKSTNM,khole_,"*",doy(1));
                %strcat(net_,'.',thisKSTNM,'.*.',doyStr(1));
                ...
                if moreDays
                    for l = 2:lDays
                        ...
                        fileStr = sprintf("%s.%s.%s.%s.%03d",net_,thisKSTNM,khole_,"*",doy(l));
