...
[evDescription,evStatus,t,eqlat,eqlon,eqdepth,~,ids,stderr,azgap,phaseTot,nMLv,...
    timerr,eqlaterr,eqlonerr,eqdeptherr,eqmagerr,magType,meanMethod,...
    locMethod,earthModel,creationTime,agencyID,evMode,scbullmag,authorID,evType] = readCat1(datetime(2010,01,01),datetime(2025,01,01),0);

eqmag = scbullmag;
[eqmag,eqmagSort] = sort(eqmag,'descend');
% eqType = eqType(eqmagSort);
...
minPhases = 8;          %20;
magerrThresh = 1;
azGapThresh = 180; %270;      %180;
spatialErrThresh = 6;   %4;
timeErrThresh = 5;      %4;
rmsThresh = 0.8;        %0.75;
minDepth = 0.1;       %5;
minMag= 0;
...
tI = t >= datetime(2000,01,01) & t <= datetime(2024,12,31) & nMLv >= minNmlv & ...
    ...
    eqdepth > minDepth & eqdeptherr > minDeptherr & (totErr < spatialErrThresh | totErr./abs(eqdepth) <= errorDepthRatio); % & ...
%contains(meanMethod,"trimmedmean"); % & strcmp(eqType,"unk") & strcmp(meanMethod,"trimmedmean");
sum(tI)
...
importAll = ~true;
if importAll
    load('~/igdata/ecuadorSensorDataTable10.mat','allSNCLs','kcmpnm','knetwk');
    lia = ismember(kcmpnm,["HHZ";"BHZ";"BLZ";"ENZ";"HNZ"]) & ismember(knetwk,["EC";"OP";"XF"]);
    goodStations = unique(allSNCLs(lia));
    goodStations = [goodStations; "IUOTAV00BHZ";...
        "IUOTAV10BHZ";...
        "IUOTAV10HHZ";...
        "IUOTAVBHZ"];
    goodStations = [goodStations; "CMBBACHHZ";...
        "CMCUMEHZ";...
        "CMFLO2HHZ";...
        "CMHELBHZ";...
        "CMMALEHZ";...
...
for i = 1:lE
    ...
    [~,b] = unique([kstnm knetwk kcmpnm],'rows');
    b = sort(b);
    keepI = false(size([kstnm knetwk kcmpnm],1),1);
    keepI(b) = true;
    MLv(~keepI) = [];
    kstnm(~keepI) = [];
    knetwk(~keepI) = [];
    ...
    [stla,~] = metaDataFromStationList(kstnm,knetwk,kcmpnm);
    stla(isnan(stla)) = [];
    sncls_ = strcat(knetwk,kstnm,kcmpnm);
    potentialStations = [potentialStations; sncls_];
    potentialStations = unique(potentialStations);
    try
        good = (strcmp(kcmpnm,"BHZ") | strcmp(kcmpnm,"HHZ") | strcmp(kcmpnm,"HNZ") | strcmp(kcmpnm,"BLZ") | strcmp(kcmpnm,"ENZ")) & ...
            (strcmp(knetwk,"EC") | strcmp(knetwk,"IU") | strcmp(knetwk,"XF") | strcmp(knetwk,"PE") | strcmp(knetwk,"CM")) & ...
            amp_ <= maxAmp & amp_ > minAmp & magEstimate >= minMag & ...
            isfinite(stla) & abs(res_) <= resThresh;
        % & ismember(sncls_,goodStations); %ismember(sncls_,badStations);
    catch
        continue;
    end
    ...
    %[stla,stlo] = metaDataFromStationList([kstnm knetwk kcmpnm]);
    ...
    theseAmps = length(amp_);
    mag_ = mean(mlvs); %median(mlvs);
    eqmag2(n) = mag_;
    ...
    allDepths = [allDepths; repmat(depth_,theseAmps,1)];
    dist = [dist; r_];
    amp = [amp; amp_];
    magMain = [magMain; repmat(mag_,theseAmps,1)];
    idMain = [idMain; repmat(id_,theseAmps,1)];

    %
    [~,goodSnclsI] = ismember(sncls,goodStations);
    diffCount = theseAmps*(theseAmps - 1)*0.5;
    if gFlag
        Gtmp = full(Gvdcc(theseAmps));
        Gtmp = Gtmp(1:end-1,:);
...
Nrows = floor(sqrt(10*ceil(nSubplots/10)));
Ncols = ceil(sqrt(10*ceil(nSubplots/Nrows)));
...
if gFlag
    ...
    inversionI = groupcounts(kstnms4) >= minObs;
    ...
    for i = 1:length(tI)
        ...
        [lia2,goodSnclsI2] = ismember(kstnms4(lia),kstnms5(inversionI)); %goodStations);
    ...
    figure(10); hold on; semilogx(rdum,attGamma100,'-','color',[0.5 0.5 0.5],'linewidth',2); grid on;
    ylim([0 6])
