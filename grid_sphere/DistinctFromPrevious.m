...
% to finding the indices of the unique elements.  If there are multiple
% instances of a value, then the lowest index at which that value is found is
% chosen.
% 
%    usage: distinctFromPrevious = DistinctFromPrevious(values)
    ...
    %stephen hack thinks its unneccessary to cast the indices as uint32
    distinctFromPrevious = find(~equalsPrevious);
