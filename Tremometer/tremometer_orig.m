function [harmPow, harmFreq, interharmPow, harmStrength] = tremometer(x,fs,nHarm,welchFlag)
if nargin < 4
    welchFlag = false;
end
...
w = kaiser(nr,20); %was previously 38th order, but i changed it to 20

if welchFlag
    nfft = 2^(nextpow2(nr)+1);
    nOverlap = floor(nr*(1-(1/8)));
    [Pxx,F] = pwelch(x,w,nOverlap,nfft,fs);
else
    [Pxx,F] = periodogram(x,w,nr,fs);	%returns the PSD estimate - the integral of the PSD is the power in the time series
end

%% pre-allocate harmonic tables
% nHarm is a user-supplied argument
% compute HSI for nHarm harmonics
harmPow = NaN(nHarm,1);
...
hps1 = Pxx; %downsample(Pxx,1);                     %Redundant - could just use Pxx
...
%%
[~,nr]=findpeaks(y,'SORTSTR','descend');
Maximum = nr(1);
Ffund = F(Maximum);
...
%%
% Adjust frequency and amplitude of candidate harmonics by finding the 
% maximum amplitude within 0.1Hz of each F in Pxx. It needs the 'else' bit 
% or it throws a subscript indices tantrum if iHarm indices are low.
