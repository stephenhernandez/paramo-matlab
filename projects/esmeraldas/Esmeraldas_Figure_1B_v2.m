...
reve_lat = -0.0809;
reve_lon = -77.65835;
[minLon,maxLon,minLat,maxLat] = deal(-80,-79.4,0.65,1.1);
...
geoshow(ax(laxes),'~/igdata/shape_files/ZonaUrbana.shp');
...
scaleRefLat2 = scaleRefLat0 + refShifter/1.5;
scaleRefLon2 = scaleRefLon0;
plot(ax(laxes),[scaleRefLon0 lonout20],[scaleRefLat0 scaleRefLat0],'k-','linewidth',4);

textShifter = -refShifter/2;
textFontSize = 20;
plot(ax(laxes),[scaleRefLon0 scaleRefLon0],[scaleRefLat0 scaleRefLat2],'k-','linewidth',4);
text(textShifter+scaleRefLon0,-textShifter+scaleRefLat2,"0","FontSize",textFontSize);

plot(ax(laxes),[lonout5 lonout5],[scaleRefLat0 scaleRefLat2],'k-','linewidth',4);
text(textShifter+lonout5,-textShifter+scaleRefLat2,"5","FontSize",textFontSize);

plot(ax(laxes),[lonout10 lonout10],[scaleRefLat0 scaleRefLat2],'k-','linewidth',4);
text(textShifter+lonout10,-textShifter+scaleRefLat2,"10","FontSize",textFontSize);

plot(ax(laxes),[lonout20 lonout20],[scaleRefLat0 scaleRefLat2],'k-','linewidth',4);
...
contour(ax(laxes),xq,yq,vq,50:10:90,'linewidth',4);
...
SS.LineWidth = 1.2;
blgd = bubblelegend('Magnitude','Location','SouthEast');
blgd.LimitLabels = {'0.8','6'};
blgd.Style = 'vertical';
blgd.NumBubbles = 2;
bubblesize(ax(laxes),[3,80]);
...
h1 = plot(ax(laxes),stloH,stlaH,'v','linewidth',2,'markeredgecolor','k','markerfacecolor','w','MarkerSize',StationMarkerSize);
...
h2 = plot(ax(laxes),stlo,stla,'^','linewidth',2,'markeredgecolor','w','markerfacecolor','k','MarkerSize',StationMarkerSize);
...
% C2lon = -79.3009142; 
% C2lat = 0.6332050;
% CC = plot(ax(laxes),[C1lon C2lon],[C1lat C2lat],'k','linewidth',2);
% CC.Color(4) = 0.5;
...
T = readtable('~/igdata/gmap-stations.txt');
% figure(1); hold on; plot(T.Var4,T.Var3,'kh'); zoom on;
h4 = plot(ax(laxes),T.Var4,T.Var3,'ko','linewidth',5); zoom on;

[legh,legIcons] = legend([h1;h2;h3;h4],'Temporary Network','RENSIG','GPS','Node','Location','SouthEast');
...
mt=sdr2mt(25,22,119); mlv = 6; focalmech(mt,-79.88+mtShifter,1.07,0.015*sqrt(mlv));

%igepn2022fztk 202203270553 0.8027 -79.7359 17.5 5.1   0.853 -79.785 17.4 4.97   23 21 121   170 72 78   0.34 17 13 12 18  202 A
mt=sdr2mt(23,21,121); mlv = 4.97; focalmech(mt,-79.84+mtShifter,1.07,0.015*sqrt(mlv));

%igepn2020gavx 202203272017 0.8454 -79.7476 15.8 5.3   0.895 -79.797 17.3 5.32   30 23 124   173 71 76   0.22 19 14 13 19  204 A
mt=sdr2mt(30,23,124); mlv = 5.32; focalmech(mt,-79.80+mtShifter,1.07,0.015*sqrt(mlv));

%igepn2022gaxb 202203272051 0.8497 -79.7575 15.4 4.6   0.832 -79.786 15.0 4.62   359 27 99   168 63 85   0.25 20 8 8 20    197 A
mt=sdr2mt(359,27,99); mlv = 4.62; focalmech(mt,-79.76+mtShifter,1.07,0.015*sqrt(mlv));
