...
[minLon,maxLon,minLat,maxLat] = deal(-79.9,-79.65,0.7,1.05); %esmeraldas 2022
%[minLon,maxLon,minLat,maxLat] = deal(-80,-79.6,0.68,1.04); %atacames 2016
%[minLon,maxLon,minLat,maxLat] = deal(-80.15,-79.5,0.7,1.3); %atacames 2016
...
%% scaleruler
refShifter = (maxLat-minLat)*(0.006/0.3);
...
% minMag = 0; [~,~,t,eqlat,eqlon,eqdepth,eqmag,id] = ...
%     readCat1(datetime(2016,11,01),datetime(2017,02,01),minMag);
...
sI = eqlat >= minLat & eqlat <= maxLat & eqlon >= minLon & eqlon <= maxLon;
t = t(sI);
eqmag = eqmag(sI);
eqlat = eqlat(sI);
eqlon = eqlon(sI);
