...
%CTX = load('CTXAll_shortWin.mat');
CTX = load('CTXAll_longWin.mat');
...
clear CTX
%TAM = load('TAMAll_shortWin.mat');
TAM = load('TAMAll_longWin.mat');
...
minAV = min([avTAM avCTX2],[],2);
maxAV = max([avTAM avCTX2],[],2);
medAV = median([avTAM avCTX2],2);
tremI = meanLinearity2 >= 0.2 & avDiff <= 200 & ...
    tremLon <= maxLon & tremLat <= maxLat & tremLon >= minLon & tremLat >= minLat & ...
    minAV >= 900 & maxAV <= 1300; % & tCommon >= datetime(2023,05,15,12,0,0) & tCommon <= datetime(2023,05,15,13,0,0);
...
SS = bubblechart(ax(laxes),tremLon(tremI),tremLat(tremI),...
    (meanLinearity2(tremI)),medAV(tremI));
bubblesize([2 12]);
...
set(ax(laxes),'ColorScale','log');
clim(ax(laxes),[900 1300]);
...
semilogy(tCommon(tremI),[avTAM(tremI) avCTX2(tremI)],'.'); zoom on; grid on; hold on;
ylabel('app. vel.');
linkaxes(ax2__,'x');
...
if exist('tBell','var')
    ...
    clear axCmp;
    axCmp(1) = subplot(221);
    plot(tBell,bellDOA(:,2),'.'); zoom on; grid on; hold on; plot(tCommon(tremI),doaCTX2(tremI),'o');
    ylabel('baz');
    legend('bell','hrndz');

    axCmp(2) = subplot(222);
    plot(tBell,bellAV,'.'); zoom on; grid on; hold on;
    plot(tCommon(tremI),avCTX2(tremI),'o');
    ylabel('app. vel.');

    axCmp(3) = subplot(223);
    plot(tBell,bellLon,'.'); zoom on; grid on; hold on;
    plot(tCommon(tremI),tremLon(tremI),'o');
    ylabel('lon'); legend('bell','hrndz');

    axCmp(4) = subplot(224);
    plot(tBell,bellLat,'.'); zoom on; grid on; hold on;
    plot(tCommon(tremI),tremLat(tremI),'o');
    ylabel('lat');
    linkaxes(axCmp,'x');

    figure(); plot(doaCTX2(tremI),avCTX2(tremI),'.'); zoom on; grid on;
    hold on;
    plot(bellDOA(:,2),bellAV,'.'); zoom on; grid on;
end
