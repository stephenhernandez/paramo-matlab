...
%%
tI = t >= minT & t <= maxT & nMag >= minNmlv & lon >= MINLON & lat <= MAXLAT & ...
...
MINDUR = 0; %0.0010; %2;
MAXDUR = 1800; %seconds
maxAmpAcc = 4e2; %1e3; %12e4;
maxAmpSeis = 2e1; %2e1; %2; %2e1; %2 for VEL/DISP, and 30 for ACC
MINAMP = 1e-2; %4e-3 for VEL/DISP, and 1e-2 for ACC
MinStations = 4; %must be at least four because it matches the number of equations we need
MAXDIST = 1e3; %8e2;
MinComponents = 3;
magFact = 3;
MINDIST = 4;
...
for i = 1:lYears
    ...
    for j = 1:lMonths
        ...
        for k = 1:lEvents
            ...
            dists_ = T.DIST;
            cmpnmI = dists_ >= MINDIST & (cmpnmI & duration_ >= MINDUR & duration_ <= MAXDUR & ...
...
badSNCLs = unique(["EC.AC07..HN";...
    ...
    "Y2.PU05..HH";...
    "Y2.PU06..HH"]);
badI = ismember(snclMain,badSNCLs);
snclMain(badI) = [];
...
for i = 1:lUniqIDs
    id_ = uniqIDs(i);
    uI = idMain == id_;
    nAmps = sum(uI); %equivalent to number of stations for this particular event
    sncls_ = snclMain(uI);
    %amp_ = medAmpMain(uI);
    amp_ = peakVecMain(uI);
    r_ = rMain(uI);
...
badI = find(Gmain(:,1)==0 | Gmain(:,2)==0 | Gmain(:,2) >= 800 | dMain < -0.1 | ...
...
figure(); 
S3 = scatter(pull(Efinal,'lon'),pull(Efinal,'lat'),magFact*exp(pull(Efinal,'mag')),...
    pull(Efinal,'depth'),'o','filled'); zoom on; grid on; axis equal;
load ~/igdata/ec_boundaries.mat
load ~/igdata/soam_noec.mat
hold on; plot(lonEC,latEC,'k-','linewidth',2); axis equal;
hold on; plot(lon_noec,lat_noec,'-','linewidth',2,'color',[0.5 0.5 0.5]); axis equal;
...
%%
[b,stats] = robustfit(Gmain(1:end-1,:),dMain(1:end-1),"bisquare",4.685,"off");
table(b(1:2),solution(1:2))
p = stats.p;
p = p(3:end);
obsThresh = find(1-p < 0.95 | ~isfinite(p))
sum(solution(3:end))
sum(b(3:end))
goodUniqueSNCLs(obsThresh)

axL = linkedPlot([],[b(3:end) solution(3:end) b(3:end)-solution(3:end)],'.',"compact");
zoom on;
hold(axL(1),"on");
text(axL(1),(1:length(b(3:end)))',b(3:end),goodUniqueSNCLs,"FontSize",14);